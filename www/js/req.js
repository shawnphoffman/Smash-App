...
require([
  ...
  'framework7'
    ...
    if (ex === undefined || (version === undefined || version !== '1.2x')) {
      console.log('New faction set');
        factions.reset(returnFactions());
        window.localStorage.smashAppVersion = '1.2x';
    }
    else {
    ...
    if (pl !== undefined) {
      ...
      players.reset(arr2, {silent:true});
    }
    ...
    $('#randomizeFactions').on('click', function() {
        ...
        for (var i = 0; i < random.length; i++){
          players.models[i].set({'factions':random[i]}, {silent:true});
        }

        players.persist();
        players.reset(JSON.parse(window.localStorage.players), {silent:true});
        players.renderView();
    ...
    $('#clearPlayers').on('click', function() {
        players.reset(null, {silent:true});
        scores.reset();
    });
